...
export const validationSchema = Joi.object({
  ...
  RESEND_API_KEY:       Joi.string().empty('').when('NODE_ENV', { is: 'production', then: Joi.required() }),
  R2_ACCOUNT_ID:        Joi.string().empty('').when('NODE_ENV', { is: 'production', then: Joi.required() }),
  R2_ACCESS_KEY_ID:     Joi.string().empty('').when('NODE_ENV', { is: 'production', then: Joi.required() }),
  R2_SECRET_ACCESS_KEY: Joi.string().empty('').when('NODE_ENV', { is: 'production', then: Joi.required() }),
